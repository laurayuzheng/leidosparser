...
def parseXML(xmlfile): 
    ...
    # iterate dict items 
    for item in root.findall('./NGBWeatherBriefing/TAF/TerminalAreaForecast/tafData'): 
    ...
    for item in root.findall('./NGBWeatherBriefing/TAF/TerminalAreaForecast/tafData/text'): 
    ...
    for item in root.findall('./NGBWeatherBriefing/TAF'): 
...
def savetoCSV(items, filename): 
    ...
    fields = ['condition', 'EncodedText', 'PlainText', 'isEmpty', 'nearestDeparture'] 
...
def main(): 
    ...
    # parse xml file with the file name 
    items = parseXML('b_copy.xml') 
  
    # store dict items in a csv file 
    savetoCSV(items, 'taf_result.csv') 
